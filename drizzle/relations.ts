...
import { categories, questions, fieldTypes, surveys, responses, dropdownOptions, answers, questionsMap, directusUsers, directusNotifications, directusCollections, directusShares, directusRoles, directusFlows, directusFolders, directusFiles, directusPolicies, directusPermissions, directusActivity, directusRevisions, directusVersions, directusSessions, directusPresets, directusDashboards, directusPanels, directusOperations, directusAccess, directusComments, directusWebhooks, directusSettings, clinicalDepartmentDailyReport, dailyPatientReport, omnisolPatients, omnisolAddresses, omnisolAdmissions, omnisolVisitations, omnisolMedicalAid, omnisolPayments, omnisolReferrals, omnisolDiagnoses, omnisolSurveysSent, omnisolAddress, omnisolPatient } from "./schema";
...
export const clinicalDepartmentDailyReportRelations = relations(clinicalDepartmentDailyReport, ({one}) => ({
	directusUser_userCreated: one(directusUsers, {
...
export const omnisolAddressesRelations = relations(omnisolAddresses, ({one, many}) => ({
	omnisolPatient: one(omnisolPatients, {
		fields: [omnisolAddresses.patientId],
		references: [omnisolPatients.patientId],
		relationName: "omnisolAddresses_patientId_omnisolPatients_patientId"
	}),
	omnisolPatients: many(omnisolPatients, {
		relationName: "omnisolPatients_addressId_omnisolAddresses_id"
	}),
}));

export const omnisolPatientsRelations = relations(omnisolPatients, ({one, many}) => ({
	omnisolAddresses: many(omnisolAddresses, {
		relationName: "omnisolAddresses_patientId_omnisolPatients_patientId"
	}),
	omnisolAddress: one(omnisolAddresses, {
		fields: [omnisolPatients.addressId],
		references: [omnisolAddresses.id],
		relationName: "omnisolPatients_addressId_omnisolAddresses_id"
	}),
...
export const omnisolPatientRelations = relations(omnisolPatient, ({one}) => ({
	omnisolAddress: one(omnisolAddress, {
		fields: [omnisolPatient.address],
		references: [omnisolAddress.id]
	}),
}));

export const omnisolAddressRelations = relations(omnisolAddress, ({many}) => ({
	omnisolPatients: many(omnisolPatient),
}));